...
「添加模型」弹窗三件事：模型 ID、上下文窗口、最大输出 Token。显示名不在这里——它在 Codex 菜单里的名字默认是**「供应商/模型 ID」**（如 `qiyuan/deepseek-v4.1`）：菜单是扁平的一份，原生模型和各供应商的模型排在一起，不带上来源就认不出同名模型属于谁。想改名去整页编辑器（改名后系统不再补前缀，那是用户自己的选择）。**「智能配置」**开关打开时，留空的长度按推荐默认值补齐（默认 1M / 128K，并在占位符里写明补什么），关掉则留空表示「未声明」；它不替用户声明能力。补齐的最大输出**不会达到你填的窗口的一半**：输出上限必须小于上下文窗口，照抄 128K 到一个小窗口上会立刻得到一对非法数值，而占位符刚刚还承诺会给一个值。能力项（输入类型、模型能力、推理档位）收在默认折叠的「高级配置」里：勾选＝声明支持，取消勾选＝明确不支持，没点过的项保持「未确认」并原样写回——未知不能被渲染成不支持，否则交集原则会把这个能力从 Codex 目录里灭掉。推理档位是一条可增删的集合（点一下设为默认），没有整页编辑器里那套预算/toggle 分支；推理参数映射是只读的：它写在网关的版本化适配器里。「重置表单」把这一层填的内容清回初始值。
...
行主操作“编辑”，次操作“测试”（只测这一行）；工具栏另给一个**测试全部**，一次把当前列表里的每个模型串行测完并把结论逐行写回「连接」列，最后用一条 Toast 汇总（N 正常 / N 失败 / N 缺 Key 未测）。行内那个按钮不改成批量：它挂在行上，按下去却测别的行才是意外；批量是要不要跑、跑哪些由列表范围决定的一个独立动作。菜单“复制 ID、纳入/移出 Codex、删除”。选中多行出现批量操作条，显示跨几家供应商、测试数和调用提示。
...
- **整页编辑器**（模型目录页或供应商模型表行的「编辑」）：同样三个字段，外加整页才放得下的东西——显示名称、供应商、压缩阈值、纳入目录开关。**新建时长度也从同一组推荐默认值起步**（弹窗那侧叫「智能配置」，这里没有开关）：留空＝未声明，而未声明的上下文根本应用不到 Codex，给两个空框等于让人先撞一次应用失败。
...
长度限制
上下文窗口 ⓘ [1000000]   最大输出 Token ⓘ [128000]
