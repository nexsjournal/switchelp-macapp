...
**抓取纪律（2026-09-21 定，起因是「一直显示正在读取仓库」）**

用户截图里插件市场停在「正在读取仓库…」。实测默认源 `anthropics/skills`：

| | 之前 | 现在 |
| --- | --- | --- |
| git tree 请求 | **21 次**（每个技能重下同一棵树，每次 160 KB） | **1 次** |
| 文件正文请求 | **394 个**（把每个技能的同目录文件全下下来，10.4 MB） | **20 个**（只读 `SKILL.md`，并发 6 路） |
| 浏览耗时 | 几分钟（几百个串行 HTTPS 请求） | **7.7 秒** |
| GitHub 接口限额占用（未认证 60 次/小时） | 每次浏览 23 次 | 每次浏览 1 次 |

三条规则，改代码时别破坏：

1. **一次 tree**：技能清单与「技能目录里还有哪些文件」都取自同一份 `list_blobs`。
2. **目录阶段不下载同目录文件的正文**，只记路径与大小（都来自 tree）；正文由 `hydrate`
   在**预览与安装**时按选中的技能补——装什么读什么。
3. **正文读取并发**（上限 6 路）。串行读 20 个 `SKILL.md` 就是 20 × 1.4 秒。

已知缺口（要在界面上说清楚，不能默默容忍）：`RepoFile.text` 是字符串，二进制附件（字体、
图片）经它就被改坏，所以超过 256 KB 的同目录文件目前**不列进清单**；`truncated` 也没有任何
界面出口——技能装不全时用户看不到提示。两件事一起修：`RepoFile` 换成能带原始字节的形状，
并把截断标到具体技能上、在详情里说明。
