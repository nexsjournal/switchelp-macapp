...
网关服务的是**发布时冻结的路由快照**，所以「改了但还没重新发布」这件事必须对用户可见。
让已纳入目录的模型退回 `Pending` 的，只有真正改变那份快照或磁盘目录的改动：

| 改动 | 为什么 |
| --- | --- |
| 模型策略（上下文、输出上限、档位、模态、内置工具声明） | 全部进快照与目录 |
| 模型显示名、目录 alias、是否纳入目录 | 目录条目本身 |
| 模型的协议覆盖、供应商的接口协议 | 快照里的 `protocol_id`，决定用哪个适配器 |
| 更换供应商的当前 Key | 快照里写死了 `credential_id` 与凭据版本 |
| 供应商改名 | 旗下模型显示名前缀跟着变＝目录内容变了 |

只改备注、或打开编辑器什么都没改就保存，**不**退回待应用：每次翻一遍模型都被要求再应用一次，
待应用条就成了噪音。
