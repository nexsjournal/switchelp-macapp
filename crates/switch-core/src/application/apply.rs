...
impl ApplyService {
    ...
    pub fn plan_apply(
        ...
        // 这里曾经额外塞一条「Chat Completions 适配属实验状态」的警告。2026-09-24 实测
        // 工具调用在真实上游上已经跑通（Codex 经适配器发起 exec 工具调用、宿主执行、模型续答，
        // 详见 docs/architecture/03-gateway-and-protocols.md 的兼容表），那条警告断言的前提
        // 不再成立，因此撤掉——留一条已经不成立的警告等于对用户说假话。
