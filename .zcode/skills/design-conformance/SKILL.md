...
description: 用实测数据（而不是凭印象）核对 Switchelp 的界面是否符合本仓库写好的设计规范。跑对比度、点击目标、溢出、重叠、字号、焦点、键盘与响应式检查，输出带证据的符合性报告。当用户要求「过一遍界面」「检查设计规范」「看看哪里不符合」「输出测试报告」「有没有遮挡/偏移/对比度问题」时使用。
---
